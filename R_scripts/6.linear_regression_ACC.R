...
for(f in 1:length(files)){
  ...
  #You don't need the filter statement here; dat$location %in% common_Cs[,1] will give you a vector of Tues and Falses and you can just slice the data frame using that.
...
for(f in 1:length(files)){
  ...
  rownames(dat) <- dat$location #change the row names so they match the location column. You can do head(dat) to see the result

  dat[common_Cs[,1],]->dat_filtered #because the rownames should contain the vector of common_Cs, this will not only select just those but also have dat_filttered in the same order.

  print(paste(names(files)[[f]], "match row names:", identical(rownames(dat_filtered), common_Cs[,1])))#I like to use identical to check everything is in the right order. It should return TRUE or FALSE. You can create a print statement that will let you know the rownames match inside of the loop
