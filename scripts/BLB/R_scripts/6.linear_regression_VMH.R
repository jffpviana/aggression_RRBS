...
files <- list.files(pattern="*VMH_filtered.csv")
names(files) <- str_match(Sys.glob("*VMH_filtered.csv"),paste0("BLB","(.*?.....)"))[,1]
# names(files)[[f]] # sample name for the current set

# load common Cs txt file as list
common_Cs <- as.data.frame(fread(file='common_Cs_VMH.txt', stringsAsFactors = FALSE, header = FALSE)) 
...
# write to csv
fwrite(linreg, file= "/rds/projects/v/vianaj-genomics-brain-development/MATRICS/bismark_methylation_extractor/filtered/linreg_VMH.csv")